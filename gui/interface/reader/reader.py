...
from PySide6.QtCore import QUrl, QEvent, QPoint, QRect, QPointF, QObject, QPropertyAnimation, \
    QEasingCurve, QParallelAnimationGroup, QSize, Slot, QTimer
from PySide6.QtCore import Qt, Signal, QSettings, Property
from PySide6.QtGui import QPixmap, QImage, QColor
from PySide6.QtWidgets import QFrame, QStackedLayout, QSlider, QStackedWidget
...
from qfluentwidgets import PushButton, ExpandSettingCard, FluentIcon, SpinBox, Slider, BodyLabel, StrongBodyLabel, \
    SubtitleLabel, SimpleCardWidget
...
# zerokku
from gui.common import SlideAniStackedWidget, PageLabel, MySlider
...
from scripts.tools import read_cbz_qpixmap
...
class ReaderSettings(KineticScrollArea):
    #view
    viewModeChanged = Signal(ReadModes)
    ...
    def __init__(self, parent=None):
        ...
        self.read_modes = {mode.value: mode for mode in ReadModes}
        ...
        self._view_mode = OptionsCard(FluentIcon.VIEW, texts=list(self.read_modes.keys()),
                                      selected=1, title="View", parent=self)
        self._fit_mode = OptionsCard(FluentIcon.FULL_SCREEN, texts=list(self.fit_modes.keys()), selected=1,
        ...
        self._setting_position = ComboBoxCard(FluentIcon.LAYOUT, "Settings position", parent=self)
        self._setting_width = SpinBoxCard(FluentIcon.FULL_SCREEN, "Settings width", parent=self)
        self._setting_width.spinBox.setRange(int(screen_geometry.width() * 0.2), int(screen_geometry.width() * 0.5))
    ...
    def _setup_ui(self):
        ...
        advanced_layout.addWidget(self._gray_scale)
        advanced_layout.addWidget(self._invert_filter)
